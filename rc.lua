...
beautiful.init("/home/mcy/.config/awesome/themes/sky/theme.lua")
...
if beautiful.wallpaper then
    for s = 1, screen.count() do
        ...
        gears.wallpaper.maximized( "/home/mcy/Pictures/bizhi2.jpg", s, true)
    end
end
-- }}}
...
tags = {}
tag_name = { "1:", "2:>_", "3:", "4:", "5:"}
tag_layout = { layouts[1], layouts[2], layouts[1], layouts[1], layouts[1]}
for s = 1, screen.count() do
    -- Each screen has its own tag table.
    tags[s] = awful.tag( tag_name, s, tag_layout)
end
-- }}}
...
volume = obvious.volume_alsa(0,"Master")
batt = obvious.battery()
temp = obvious.temp_info()
-- }}}
...
function update_netstat()
    ...
    if netif then
        ...
        text = '↓<span color="blue">'.. down ..'</span> ↑<span color="yellow">'.. up ..'</span>'
    else
...
memwidget:buttons(awful.util.table.join(
    awful.button({ }, 1, function () awful.util.spawn_with_shell("xfce4-taskmanager") end),
    awful.button({ }, 3, function () awful.util.spawn_with_shell("useasroot xfce4-taskmanager") end)
))
-- }}}
...
-- Register widget
vicious.register(cpuwidget, vicious.widgets.cpu, ' cpu <span color="purple">$1%</span>')
-- }}}

-- {{{ backlight widget
backlight = wibox.widget.textbox()
-- function
local fbri_max = io.open("/sys/class/backlight/intel_backlight/max_brightness","r")
bri_max = fbri_max:read()
fbri_max:close()
function bri_now()
    local fbri_n = io.open("/sys/class/backlight/intel_backlight/brightness","r")
    local bri_n = fbri_n:read()
    local brightness = math.floor( bri_n / bri_max * 100 )
    backlight:set_markup(" "..brightness)
    fbri_n:close()
    return brightness
end

local brightness = bri_now()

--buttons 
backlight:buttons(awful.util.table.join(
   awful.button({ }, 1, function () bri_now() end), --左击
   awful.button({ }, 4, function () awful.util.spawn("useasroot liangdu 10+ &", false) bri_now() end), --上滚
   awful.button({ }, 5, function () awful.util.spawn("useasroot liangdu 10- &", false) bri_now() end) --下滚
    ))
-- }}}
...
tool = {
    { "matlab(&M)", "xfce4-terminal -e matlab"},
}
...
awful.rules.rules = {
    ...
    -- set terminal to alwas map on tags number 2  of screen 1.
